...
struct RpcProvider::Param
{
    const muduo::net::TcpConnectionPtr &conn_;
    google::protobuf::Message *response_;
    bool keep_alive_;
    Param(const muduo::net::TcpConnectionPtr &conn, google::protobuf::Message *response, bool keep_alive) : conn_(conn), response_(response), keep_alive_(keep_alive) {}
};
...
void RpcProvider::Run()
{
    ...
    for (auto &sp : service_map_)
    {
        ...
        for (auto &mp : sp.second.method_map_)
        {
            ...
            meta.set_ip(ip);
            meta.set_port(port);
            meta.set_keep_alive(keep_alive_method_set_.count(mp.first));
            // 创建方法节点
            zookeeper_client.Create(method_path, meta.SerializeAsString());
        }
    }
                            ...
                            muduo::Timestamp)
{
    ...
    bool keep_alive = keep_alive_method_set_.count(method_name);
    ...
    google::protobuf::Closure *done = google::protobuf::NewCallback<RpcProvider,
                                                                    Param>(this,
                                                                           ...
                                                                           Param(conn, response, keep_alive));
    // 调用RPC服务对象，执行RPC方法，得到响应消息, 并将消息返回给客户端
    TheRpcController *controller = new TheRpcController;
...
void RpcProvider::SendRpcResponse(Param param)
{
    std::string response_str;
    if (param.response_->SerializeToString(&response_str))
    {
        param.conn_->send(response_str);
    }
    else
    {
        LOG_ERROR << "serialize response string error!";
    }
    if (!param.keep_alive_)
    {
        param.conn_->shutdown();
    }
}
